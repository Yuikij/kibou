...
```
###### 可调用的工具
1. browser_use（与网页浏览器交互，执行各种操作，如导航、元素交互、内容提取和标签页管理。搜索类优先考虑此工具。）
2. text_file_operator（对文本文件（包括 md、html、css、java 等）执行各种操作）
3. terminate（erminate the current execution step with a comprehensive summary message.）
