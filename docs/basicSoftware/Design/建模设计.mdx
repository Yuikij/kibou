...
| 序号| 表示  | 关系  |解析|
|---|---|---|---|
| 1| -->   | 关联 Association   |类之间存在关系|
| 2| o-- |  聚合 Aggregation   |关联的一种变体，具有整体和部分的概念，但是部分可以独立存在，和关联的实现一样|
| 3| ..>  | 依赖 Dependency    |更弱的关联，可能不是成员变量，而是函数参数|
| 4| *--  | 组合 Composition   |关联的一种变体，具有整体和部分的概念，但是不部分可以独立存在，更强的聚合，控制着被聚合元素的生命周期|
| 5| \<|--  | 泛化/继承 Generalization/Inheritance   |接口实现|
| 6| ..|>  | 实现 Implementation   |父类和子类的关系|
