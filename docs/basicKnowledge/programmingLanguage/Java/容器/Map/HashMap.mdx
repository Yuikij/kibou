...
* #### 1.8 的索引重新计算
  前提：数组长度 `length = 2^m`，扩容后 `newLength = 2^(m+1)`。`h` 为用于寻址的哈希值（见上文混合后的 hash）。

  规则：
  - 旧索引：`oldIndex = h & (length - 1)`
  - 判断新增位：`flag = h & length`
  - 新索引：`newIndex = (flag == 0) ? oldIndex : oldIndex + length`

  直观理解：扩容后掩码从 `0...0111`（共 m 位 1）变为 `1...0111`（在更高一位再加一个 1）。低 m 位的结果不变，新增的这一位是否为 1 决定是否加上 `length`。

  示例：
  - `length = 4 (100b)`，扩容后 `newLength = 8 (1000b)`
  - `h = 14 (1110b)`
  - `oldIndex = 1110 & 011 = 010(2) = 2`
  - `flag = 1110 & 100 = 100 ≠ 0` → `newIndex = 2 + 4 = 6`
  - 等价表述：旧结果 `010`，扩容后新增位为 1，变为 `110`，即 `010 + 100`
